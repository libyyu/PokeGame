...
#include "../lua/lua51/src/lua.h"
#include "../lua/lua51/src/lualib.h"
#include "../lua/lua51/src/lauxlib.h"
...
static int sizei (const LpegInstruction *i) {
...
static void printpatt (LpegInstruction *p) {
...
*/
static int verify (lua_State *L, LpegInstruction *op, const LpegInstruction *p,
                   LpegInstruction *e, int postable, int rule) {
  ...
  while (p != e) {
    switch ((Opcode)p->i.code) {
      ...
      case IChar:
      case ISet: {
        const LpegInstruction *next = p + sizei(p);
...
static void checkrule (lua_State *L, LpegInstruction *op, int from, int to,
...
static LpegInstruction *getpatt (lua_State *L, int idx, int *size);


static void check2test (LpegInstruction *p, int n) {
...
*/
static void invert (LpegInstruction *p, int e) {
  int i;
  for (i = 0; i < e; i++, e--) {
    LpegInstruction temp = p[i];
    p[i] = p[e];
    p[e] = temp;
  }
}
...
** to the 'left'
*/
static void rotate (LpegInstruction *p, int e, int n) {
  invert(p, n - 1);
  invert(p + n, e - n);
  invert(p, e);
}
...
static int skipchecks (LpegInstruction *p, int up, int *pn) {
  int i, n = 0;
...
static void optimizecaptures (LpegInstruction *p) {
  int i;
  int limit = 0;
...
static int target (LpegInstruction *p, int i) {
  while (p[i].i.code == IJmp)  i += p[i].i.offset;
  return i;
}


static void optimizejumps (LpegInstruction *p) {
  int i;
  for (i = 0; p[i].i.code != IEnd; i += sizei(p + i)) {
    if (isjmp(p + i))
...
static void optimizechoice (LpegInstruction *p) {
...
** LpegInstruction, which must be a check.
*/
static int isheadfail (LpegInstruction *p) {
...
#define checkpattern(L, idx) ((LpegInstruction *)luaL_checkudata(L, idx, PATTERN_T))
...
#define copypatt(p1,p2,sz)	memcpy(p1, p2, (sz) * sizeof(LpegInstruction));

#define pattsize(L,idx)		(lua_objlen(L, idx)/sizeof(LpegInstruction) - 1)


static int addpatt (lua_State *L, LpegInstruction *p, int p1idx) {
  LpegInstruction *p1 = (LpegInstruction *)lua_touserdata(L, p1idx);
...
static void setinstaux (LpegInstruction *i, Opcode op, int offset, int aux) {
  assert(aux <= MAXAUX);
  i->i.code = op;
  i->i.offset = offset;
  i->i.aux = aux;
...
static LpegInstruction *newpatt (lua_State *L, size_t n) {
  LpegInstruction *p;
  if (n >= MAXPATTSIZE - 1)
    luaL_error(L, "pattern too big");
  p = (LpegInstruction *)lua_newuserdata(L, (n + 1) * sizeof(LpegInstruction));
...
static void fillcharset (LpegInstruction *p, Charset cs) {
  switch (p[0].i.code) {
    case ISet: {
    ...
    default: {  /* any char may start unhandled LpegInstructions */
      loopset(i, cs[i] = 0xff);
      break;
    }
  }
...
static enum charsetanswer tocharset (LpegInstruction *p, CharsetTag *c) {
  if (isfixcheck(p)) {
    fillcharset(p, c->cs);
...
static LpegInstruction *newcharset (lua_State *L) {
  LpegInstruction *p = newpatt(L, CHARSETINSTSIZE);
  p[0].i.code = ISet;
  p[0].i.offset = 0;
...
static int set_l (lua_State *L) {
  ...
  if (l == 1)
    ...
    LpegInstruction *p = newcharset(L);
    while (l--) {
...
static LpegInstruction *fix_l (lua_State *L, int t) {
  LpegInstruction *p;
  int i;
  ...
  setinst(p, ICall, i);  /* first LpegInstruction calls initial rule */
  /* correct calls */
...
static LpegInstruction *any (lua_State *L, int n, int extra, int *offsetp) {
  int offset = offsetp ? *offsetp : 0;
  LpegInstruction *p = newpatt(L, (n - 1)/UCHAR_MAX + extra + 1);
  LpegInstruction *p1 = p + offset;
...
static int concat_l (lua_State *L) {
  /* p1; p2; */
  int l1, l2;
  LpegInstruction *p1 = getpatt(L, 1, &l1);
  LpegInstruction *p2 = getpatt(L, 2, &l2);
  if (isfail(p1) || issucc(p2))
    ...
    LpegInstruction *op = newpatt(L, l1 + l2);
    LpegInstruction *p = op + addpatt(L, op, 1);
    addpatt(L, p, 2);
    optimizecaptures(op);
  }
  return 1;
...
static int diff_l (lua_State *L) {
  ...
  if (tocharset(p1, &st1) == ISCHARSET && tocharset(p2, &st2) == ISCHARSET) {
    ...
    LpegInstruction *p = newpatt(L, l2 + 1 + l1);
    p += addpatt(L, p, 2);
    ...
    LpegInstruction *p = newpatt(L, 1 + l2 + 1 + l1);
    LpegInstruction *pi = p;
...
static int unm_l (lua_State *L) {
  LpegInstruction *p = getpatt(L, 1, NULL);
  if (isfail(p)) {  /* -false? */
    newpatt(L, 0);  /* true */
    return 1;
  }
  else if (issucc(p)) {  /* -true? */
    LpegInstruction *p1 = newpatt(L, 1);  /* false */
    setinst(p1, IFail, 0);
    return 1;
  }
  else {  /* -A == '' - A */
...
static int nocalls (const LpegInstruction *p) {
...
static int firstpart (LpegInstruction *p, int l) {
  if (istest(p)) {
...
static LpegInstruction *auxnew (lua_State *L, LpegInstruction **op, int *size,
                                         int extra) {
...
static int nofail (LpegInstruction *p, int l) {
  int i;
...
static int interfere (LpegInstruction *p1, int l1, CharsetTag *st2) {
...
static LpegInstruction *basicUnion (lua_State *L, LpegInstruction *p1, int l1,
                                int l2, int *size, CharsetTag *st2) {
  LpegInstruction *op;
  CharsetTag st1;
  tocharset(p1, &st1);
  if (st1.tag == ISCHARSET && st2->tag == ISCHARSET) {
    LpegInstruction *p = auxnew(L, &op, size, CHARSETINSTSIZE);
    setinst(p, ISet, 0);
    loopset(i, p[1].buff[i] = st1.cs[i] | st2->cs[i]);
  }
  else if (exclusive(&st1, st2) || isheadfail(p1)) {
    LpegInstruction *p = auxnew(L, &op, size, l1 + 1 + l2);
    copypatt(p, p1, l1);
    check2test(p, l1 + 1);
    p += l1;
    setinst(p++, IJmp, l2 + 1);
    ...
    LpegInstruction *p = auxnew(L, &op, size, 1 + l1 + 1 + l2);
    setinst(p++, IChoice, 1 + l1 + 1);
    copypatt(p, p1, l1); p += l1;
...
static LpegInstruction *separateparts (lua_State *L, LpegInstruction *p1, int l1,
                                   int l2, int *size, CharsetTag *st2) {
  ...
  if (sp == 0)  /* first part is entire p1? */
    ...
    *size = end;
    ...
    LpegInstruction *p;
    int init = *size;
    int end = init + sp + 1;  /* needs one extra LpegInstruction (choice) */
    int sizefirst = sizei(p1);  /* size of p1's first LpegInstruction (the test) */
    *size = end;
    p = separateparts(L, p1 + sp, l1 - sp, l2, size, st2);
    copypatt(p + init, p1, sizefirst);  /* copy the test */
    (p + init)->i.offset++;  /* correct jump (because of new LpegInstruction) */
    init += sizefirst;
...
static int repeatcharset (lua_State *L, Charset cs, int l1, int n) {
  /* e; ...; e; span; */
  int i;
  LpegInstruction *p = newpatt(L, n*l1 + CHARSETINSTSIZE);
  for (i = 0; i < n; i++) {
    p += addpatt(L, p, 1);
  }
  setinst(p, ISpan, 0);
...
static LpegInstruction *repeatheadfail (lua_State *L, int l1, int n) {
  /* e; ...; e; L2: e'(L1); jump L2; L1: ... */
  int i;
  LpegInstruction *p = newpatt(L, (n + 1)*l1 + 1);
  LpegInstruction *op = p;
  for (i = 0; i < n; i++) {
    p += addpatt(L, p, 1);
  }
...
static LpegInstruction *repeats (lua_State *L, LpegInstruction *p1, int l1, int n) {
  /* e; ...; e; choice L1; L2: e; partialcommit L2; L1: ... */
  int i;
  LpegInstruction *op = newpatt(L, (n + 1)*l1 + 2);
  LpegInstruction *p = op;
...
static void optionalheadfail (lua_State *L, int l1, int n) {
  LpegInstruction *op = newpatt(L, n * l1);
  LpegInstruction *p = op;
  int i;
  for (i = 0; i < n; i++) {
    p += addpatt(L, p, 1);
...
static void optionals (lua_State *L, int l1, int n) {
  ...
  int i;
  LpegInstruction *op = newpatt(L, n*(l1 + 1) + 1);
  LpegInstruction *p = op;
...
static int capture_aux (lua_State *L, int kind, int labelidx) {
  ...
  if (lc == l1) {  /* got whole pattern? */
    /* may use a IFullCapture LpegInstruction at its end */
    LpegInstruction *p = newpatt(L, l1 + 1);
    ...
    LpegInstruction *op = newpatt(L, 1 + l1 + 1);
    LpegInstruction *p = op;
...
static int capconst_l (lua_State *L) {
  int i, j;
  int n = lua_gettop(L);
  LpegInstruction *p = newpatt(L, n > 1 ? n + 2 : n);
...
static const luaL_Reg metapattreg[] = {
  {"__add", union_l},
  {"__pow", star_l},
  {"__sub", diff_l},
  {"__mul", concat_l},
