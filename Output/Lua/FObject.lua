...
function FClass._define(clsType, className, ...)
	...
	local typeName = "\"" .. clazz.className .. "(".. tostring(clazz) .. ")\""
	--warn("******************", typeName)
...
function FClass.new(class, ...)
	...
	--<=
	meta.__le = function(t, tb)
		warn("111111111LLle")
		local m = getmetatable(t)
		local func = m.__functionsMap.lastFuncs["__le"]
		if not func then
			error("attempt to compare with table value")
		else
			return func(t, tb)
		end 
	end
	meta.__eq = function(t, tb)
		warn("222222222")
		local m = getmetatable(t)
		local func = m.__functionsMap.lastFuncs["__eq"]
		if not func then
			error("attempt to compare with table value")
		else
			return func(t, tb)
	...
	meta.__lt = function(t, tb)
		warn("__lt Object")
		local m = getmetatable(t)
		local func = m.__functionsMap.lastFuncs["__lt"]
		if not func then
			error("attempt to compare with table value")
		else
			return func(t, tb)
	...
	meta.__add = function(t, v)
		warn("___add Object")
		local m = getmetatable(t)
		local func = m.__functionsMap.lastFuncs["__add"]
		if not func then
			error("attempt to calc with table value")
		else
			return func(t, v)
		end 
	end
	...
	meta.__tostring = function (t)
		local m = getmetatable(t)
		local func = m.__functionsMap.lastFuncs["toString"]
