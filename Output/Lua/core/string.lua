...
function string:startswith(substr, plain)          
    if substr == nil then 
        return nil, "the string or the sub-stirng parameter is nil"          
    end          
    if self:find(substr, 1, plain) ~= 1 then              
        return false         
    else              
        return true         
    end      
...
function string:endswith(substr, plain)          
    if substr == nil then              
        return nil, "the string or the sub-string parameter is nil"          
    end          
    local str_tmp = self:reverse()      
    local substr_tmp = substr:reverse()          
    if str_tmp:find(substr_tmp, plain) ~= 1 then              
        return false         
    else              
        return true          
    end     
...
function string.split(self, delimiter)
    ...
    local arr = {}
    while true do
        local pos = string.find(self, delimiter, start, true)
        if not pos then
            break
        end
        table.insert (arr, string.sub (self, start, pos - 1))
        start = pos + string.len (delimiter)
    end
    table.insert (arr, string.sub (self, start))
    return arr
end
