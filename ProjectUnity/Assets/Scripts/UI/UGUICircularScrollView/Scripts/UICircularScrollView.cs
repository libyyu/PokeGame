...
namespace CircularScrollView
{
    ...
    public class UIUtils
    {
        ...
        public static void GetWidthAndHeight(Transform tf, ref float w, ref float h)
        {
            Vector3[] contentC = new Vector3[4];
            tf.GetComponent<RectTransform>().GetWorldCorners(contentC);
            w = Mathf.Abs(contentC[0].x - contentC[2].x);
            h = Mathf.Abs(contentC[0].y - contentC[1].y);
        }
    }
    ...
    public class UICircularScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        protected virtual void InitCellTemplate()
        {
            /* Cell 处理 */
            ...
            UIUtils.SetActive(m_CellGameObject, false);
            ...
            //记录 Cell 信息
            float CellWidget = cellRectTrans.rect.height;
            float CellHeight = cellRectTrans.rect.width;
            UIUtils.GetWidthAndHeight(m_CellGameObject.transform, ref CellWidget, ref CellHeight);
        ...
        public virtual void ShowList(int num)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                cell.transform.GetComponent<RectTransform>().localPosition = new Vector3(cellInfo.pos.x, cellInfo.pos.y, 0);
        ...
        protected virtual GameObject GetPoolsObj()
        {
            ...
            //else if (cell.transform != m_Content.transform)
            //{
